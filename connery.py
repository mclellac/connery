...
if sys.version_info < (2, 7):
    stderr('Error: Requires Python 2.7 or later. Try python2.7 connery')
    sys.exit(1)
...
from connery.__init__ import run, __version__
from connery.config import Config, create_config, ConfigurationError, wizard
import connery.tools as tools
import connery.web

homedir = os.path.join(os.path.expanduser('~'), '.connery')


def enumerate_configs(extension='.cfg'):
    configfiles = []
    if os.path.isdir(homedir):
        connery_dotdirfiles = os.listdir(homedir)  # Preferred
        for item in connery_dotdirfiles:
...
def main(argv=None):
    ...
    try:
        parser = argparse.ArgumentParser(description='Connery IRC Bot',
                                         usage='%(prog)s [options]')
        parser.add_argument('-c', '--config', metavar='filename',
                            help='use a specific configuration file')
        parser.add_argument("-d", '--fork', action="store_true",
                            dest="deamonize", help="Deamonize connery")
        parser.add_argument("-q", '--quit', action="store_true", dest="quit",
                            help="Gracefully quit Connery")
        ...
        try:
            ...
            if os.getuid() == 0 or os.geteuid() == 0:
                stderr('Error: Do not run Connery with root privileges.')
                sys.exit(1)
        except AttributeError:
            # Windows
            if os.environ.get("USERNAME") == "Administrator":
                stderr('Error: Do not run Connery as Administrator.')
                sys.exit(1)
        ...
        if opts.version:
            ...
            print('Connery %s (running on python %s)' % (__version__, py_ver))
            print('http://connery.dftba.net/')
            return
        elif opts.wizard:
        ...
        if opts.config is None:
            pid_file_path = os.path.join(pid_dir, 'connery.pid')
        else:
            basename = os.path.basename(opts.config)
            if basename.endswith('.cfg'):
                basename = basename[:-4]
            pid_file_path = os.path.join(pid_dir, 'connery-%s.pid' % basename)
        if os.path.isfile(pid_file_path):
            ...
            if old_pid is not None and tools.check_pid(old_pid):
                if not opts.quit and not opts.kill:
                    stderr('There\'s already a Connery instance running with this config file')
                    stderr('Try using the --quit or the --kill options')
                    sys.exit(1)
                elif opts.kill:
                    stderr('Killing the connery')
                    os.kill(old_pid, signal.SIGKILL)
                    sys.exit(0)
                elif opts.quit:
                    stderr('Signaling Connery to stop gracefully')
        ...
        # Step Five: Initialise And Run connery
        run(config_module)
