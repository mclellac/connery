.. Connery IRC Bot documentation master file, created by
...
Getting started: Your functions, ``connery``, and ``trigger``
============================================================

At its most basic, writing a Connery module involves creating a Python file with
some number of functions in it. Each of these functions will be passed a 
``Connery`` object (``Phenny`` in *1.x* and ``Jenni`` in *2.x*) and a ``Trigger``
object (``CommandInput`` in *1.x* and *2.x*). By convention, these are named
``phenny`` and ``input`` in *1.x*, ``jenni`` and ``input`` in *2.x*, 
``connery`` and ``trigger`` in *3.x*, and ``bot`` and ``trigger`` from version
*4.0* onward. For the purposes of this guide, the *4.0* names will be used.

Your modules
------------

A Connery module contains one or more ``callable``\s. It may optionally contain
...
done with decorators, imported from :py:mod:`connery.module`. It may also be
...
.. py:method:: callable(bot, trigger)
    ...
    This is the general function format, called by Connery when a command is
    ...
    is that it be callable with a ``Connery`` object and a ``Trigger`` object,
    as noted above. Usually, methods of the Connery object will be used in reply
    ...
    :py:mod:`connery.module`, except in version *3.2* in which they are defined
    as attributes of the ``Connery`` class.
    ...
    .. py:attribute:: commands
        ...
        *See also:* :py:func:`connery.module.commands`

        A list of commands which will trigger the function. If the Connery instance
    ...
    .. py:attribute:: rule
        ...
        *See also:* :py:func:`connery.module.rule`

        A regular expression which will trigger the function. If the Connery
    ...
    .. py:attribute:: rate
        ...
        *See also:* :py:func:`connery.module.rate`
...
.. py:method:: setup(connery)
    ...
    the Connery object's messaging functions is undefined.

    Throwing an exception from this function (such as a `ConfigurationError
    <#connery.config.ConfigurationError>`_) will prevent any callables in the
...
.. py:method:: shutdown(connery)

    *Availability: 4.1+*

    This is an optional function of a module, which will be called while the
    Connery is quitting. Note that this normally occurs after closing connection
    to the server, so the behavior of the Connery object's messaging functions
...
.. py:function:: quit(message)
    ...
    Connery will notify modules that it is quitting should the modules have
    a ``shutdown`` method.
...
.. py:attribute:: nick
    ...
    Connery's current nick. Changing this while Connery is running is unsupported.

.. py:attribute:: name

    Connery's "real name", as used for whois.

.. py:attribute:: password

    Connery's NickServ password

.. py:attribute:: channels

    A list of Connery's initial channels. This list will initially be the same
...
.. py:function:: write(args, text=None)
    ...
    In other words, both ``connery.write(('PRIVMSG',), 'Hello, world!')``
    and ``connery.write(('PRIVMSG', ':Hello, world!'))`` will send
   ...
   .. py:function:: msg(recipient, text, max_messages=1)
    ...
    Connery from being caught in an infinite loop with another bot, or being
    used to spam.
    ...
    determinable by Connery (though you can generally rely on 400 bytes
    making it through).
...
.. py:function:: debug(tag, text, level)
    ...
    *Availability: 3+*
    ...
    Send ``text`` to Connery's configured ``debug_target``. This can be either
    an IRC channel (starting with ``#``) or ``stdio``. Suppress the message
    if the given ``level`` is lower than Connery's configured ``verbose``
    setting. Acceptable values for ``level`` are ``'verbose'`` (only send if
    Connery is in verbose mode), ``'warning'`` (send if Connery is in verbose
...
.. autoclass:: connery.trigger.Trigger
   :members:

More advanced: ``db`` and ``config``
====================================

The ``connery`` object has, among others, the attributes ``db`` and
``config``. These can be used for a number of functions and features.

The ``ConneryDB`` class
----------------------

.. automodule:: connery.db
   :members:

The ``Config`` class
--------------------

.. automodule:: connery.config
   :members:
   :undoc-members:
...
.. automodule:: connery.web
   :members:

connery.tools
------------

.. automodule:: connery.tools
   :members:

.. automodule:: connery.tools.time
   :members:

.. automodule:: connery.tools.calculation
   :members:

connery.module
-------------
.. automodule:: connery.module
   :members:

connery.formatting
-----------------
.. automodule:: connery.formatting
   :members:
   :undoc-members:
