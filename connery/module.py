# coding=utf8
"""This module is meant to be imported from connery modules.

It defines the following decorators for defining connery callables:
connery.module.rule
connery.module.thread
connery.module.commands
connery.module.nickname_commands
connery.module.priority
connery.module.event
connery.module.rate
connery.module.example
"""
...
import connery.test_tools
import functools

NOLIMIT = 1
"""Return value for ``callable``\s, which supresses rate limiting for the call.

*Avalability: 4.0+; available as ``Connery.NOLIMIT`` in 3.2*
...
def rule(value):
    ...
    If the Connery instance is in a channel, or sent a PRIVMSG, where a string
...
def event(*event_list):
    ...
    (More details can be found in RFC 1459.) When the Connery bot is sent one of
...
def rate(value):
    ...
    on the admin list in Connery’s configuration are exempted from rate limits.
