# coding=utf8
"""
tell.py - Connery Tell and Ask Module
...
import threading
import sys
from connery.tools import Identifier, iterkeys
from connery.tools.time import get_timezone, format_time
from connery.module import commands, nickname_commands, rule, priority, example
...
@example('Connery, tell Embolalia he broke something again.')
def f_remind(bot, trigger):
    ...
    if len(tellee) > 20:
        return bot.reply('That nickname ish too long.')
    if tellee == bot.nick:
    ...
    if not tellee in (Identifier(teller), bot.nick, 'me'):
        ...
        response = "I'll pash that on when %s ish around." % tellee

        bot.reply(response)
    elif Identifier(teller) == tellee:
        bot.say('You can %s yourshelf that.' % verb)
    else:
        bot.say("Hey, I'm not as shtupid as a woman you know!")
...
def message(bot, trigger):
    ...
    if reminders[maximum:]:
        bot.say('Further meshages shent privately')
        for line in reminders[maximum:]:
            bot.msg(tellee, line)
