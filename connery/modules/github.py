# coding=utf8
"""
github.py - Connery Github Module
...
import json
from connery import web, tools
from connery.module import commands, rule, NOLIMIT
import os
import re
...
def configure(config):
    ...
    | repo | embolalia/connery | The GitHub repo you're working from. |
    """
    chunk = ''
    if config.option('Configuring github issue reporting and searching module', False):
        ...
        config.interactive_add('github', 'repo', 'Github repository', 'embolalia/connery')
    return chunk


def setup(bot):
    if not bot.memory.contains('url_callbacks'):
        bot.memory['url_callbacks'] = tools.ConneryMemory()
