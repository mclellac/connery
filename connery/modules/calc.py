# coding=utf8
"""
calc.py - Connery Calculator Module
...
def c(bot, trigger):
    ...
    try:
        ...
        result = "Division by zero is not shupported in this universe, sho fek off."
...
def py(bot, trigger):
    ...
    if not trigger.group(2):
        return bot.say("Need an expreshion to evaluate")
...
def wa(bot, trigger):
    ...
    if not trigger.group(2):
        return bot.reply("No shearch term.")
    query = trigger.group(2)
    ...
    try:
        ...
        return bot.say('[WOLFRAM ERROR] Requesht timed out')
    if answer:
        ...
        if(len(waOutputArray) < 2):
            if(answer.strip() == "Couldn't grab reshultsh from jshon shtringified precioussh."):
                # Answer isn't given in an IRC-able format, just link to it.
                bot.say('[WOLFRAM]Couldn\'t dishplay ansher, try http://www.wolframalpha.com/input/?i=' + query.replace(' ', '+'))
            else:
