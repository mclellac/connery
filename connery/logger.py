...
class ChannelOutputFormatter(logging.Formatter):
    ...
    def formatException(self, exc_info):
        ...
        # that's fine because Connery will strip it back out anyway
...
def setup_logging(bot):
    ...
    logger = logging.getLogger('connery')
...
def get_logger(name=None):
    ...
    This is equivalent to `logging.getLogger('connery.modules.' + name)` when
    name is given, and `logging.getLogger('connery')` when it is not. The latter
    case is intended for use in Connery's core; modules should call
    `get_logger(__name__)` to get a logger."""
    if name:
        return logging.getLogger('connery.modules.' + name)
    else:
        return logging.getLogger('connery')
