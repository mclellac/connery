...
from connery import tools
import connery.irc as irc
from connery.db import ConneryDB
from connery.tools import (stderr, Identifier, get_command_regexp, iteritems,
                          itervalues)
import connery.tools.jobs
from connery.trigger import Trigger
import connery.module as module
from connery.logger import get_logger
...
class Connery(irc.Bot):
    NOLIMIT = module.NOLIMIT

    def __init__(self, config):
        irc.Bot.__init__(self, config.core)
        self.config = config
        """The ``Config`` for the current Connery instance."""
        self.doc = {}
        """
        ...
        self.db = ConneryDB(config)
        """The bot's database."""

        self.memory = tools.ConneryMemory()
        """
        A thread-safe dict for storage of runtime data to be shared between
        modules. See `ConneryMemory <#tools.Connery.ConneryMemory>`_
        """

        self.scheduler = connery.tools.jobs.JobScheduler(self)
    ...
    JobScheduler = connery.tools.jobs.JobScheduler
    Job = connery.tools.jobs.JobScheduler

    def setup(self):
        stderr("\nWelcome to Connery. Loading modules...\n\n")
        self.callables = set()
    ...
    @staticmethod
    def is_callable(obj):
        """Return true if object is a connery callable.
        ...
        as a connery callable.

        """
        if not callable(obj):
            # Check is to help distinguish between connery callables and objects
    ...
    @staticmethod
    def is_shutdown(obj):
        """Return true if object is a connery shutdown method.
    ...
    def register(self, variables):
        """Register all connery callables.

        With the ``__dict__`` attribute from a Connery module, update or add the
        trigger commands and rules, to allow the function to be triggered, and
        shutdown methods, to allow the modules to be notified when connery is
        quitting.
    ...
    def unregister(self, variables):
        """Unregister all connery callables in variables, and their bindings.
        ...
        Args:
        variables -- A list of callable objects from a connery module.
        ...
        for func in self.callables:
            ...
            if hasattr(func, 'interval'):
                for interval in func.interval:
                    job = connery.tools.jobs.Job(interval, func)
                    self.scheduler.add_job(job)

    class ConneryWrapper(object):
        def __init__(self, connery, trigger):
            ...
            object.__setattr__(self, '_bot', connery)
    ...
    def call(self, func, connery, trigger):
        nick = trigger.nick
        ...
        try:
            exit_code = func(connery, trigger)
        except Exception:
            exit_code = None
            self.error(trigger)
    ...
    def dispatch(self, pretrigger):
        ...
        for priority in ('high', 'medium', 'low'):
            ...
            for regexp, funcs in items:
                ...
                wrapper = self.ConneryWrapper(self, trigger)
    ...
    def cap_req(self, module_name, capability, failure_callback):
        """Tell Connery to request a capability when it starts.
        ...
        The arguments will be a `Connery` object, and the capability which was
