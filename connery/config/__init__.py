...
The config class is an abstraction class for accessing the active Connery
configuration file.

The Connery config file is divided to sections, and each section contains keys
...
the Connery core. Modules are allowed to read those, but must not change them.
...
through creating settings for the Connery core, the settings database, and any
...
from connery.tools import iteritems
import os
import sys
try:
    import ConfigParser
...
class Config(object):
    def __init__(self, filename, load=True, ignore_errors=False):
        ...
        if load:
            ...
            if not self.parser.has_option('core', 'user'):
                self.parser.set('core', 'user', 'connery')
            if not self.parser.has_option('core', 'name'):
                self.parser.set('core', 'name',
                                'Connery Embosbot, http://connery.dftba.net')
    ...
    def enumerate_modules(self, show_all=False):
        ...
        *Availability: 4.0+*
        ...
        directory, followed by `~/.connery/modules`, and then through the extra
...
def wizard(section, config=None):
    dotdir = os.path.expanduser('~/.connery')
...
def check_dir(create=True):
    dotdir = os.path.join(os.path.expanduser('~'), '.connery')
    if not os.path.isdir(dotdir):
        if create:
            print('Creating a config directory at ~/.connery...')
            try:
                os.makedirs(dotdir)
            except Exception as e:
                print('There was a problem creating %s:' % dotdir, file=sys.stderr)
                print('%s, %s' % (e.__class__, str(e)), file=sys.stderr)
                print('Please fix this and then run Connery again.', file=sys.stderr)
                sys.exit(1)
        else:
