...
f = open('connery-%s.tar' % version, 'w')
repo.archive(f, prefix='connery-%s/' % version)
f.close()

print 'Building spec file..'
spec_in = open('connery.spec.in', 'r')
spec_out = open('connery.spec', 'w')
for line in spec_in:
...
cmdline = 'rpmbuild --define="%_specdir @wd@" --define="%_rpmdir @wd@" --define="%_srcrpmdir @wd@" --define="%_sourcedir @wd@" -ba connery.spec'.replace('@wd@', os.getcwd())
p = call(cmdline, shell=True)
for item in os.listdir('noarch'):
    os.rename(os.path.join('noarch', item), item)
print 'Cleaning...'
os.removedirs('noarch')
os.remove('connery.spec')
os.remove('connery-%s.tar' % version)
print 'Done'
