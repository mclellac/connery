...
def main():
    if connery.__version__.split('.', 1)[0] != '5':
        print('Must have Connery 5 installed to run migration script.')
        return
    if len(sys.argv) != 2:
        print('Usage: ./update_db.py /path/to/config')
    config = connery.config.Config(sys.argv[1])
    ...
    new_db = connery.db.ConneryDB(config)
