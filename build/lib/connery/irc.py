...
from connery.tools import stderr, Identifier
from connery.trigger import PreTrigger, Trigger
try:
    import select
    import ssl
...
class Bot(asynchat.async_chat):
    def __init__(self, config):
        ...
        """Connery's current ``Identifier``. Changing this while Connery is running is
        untested."""
        self.user = config.user
        """Connery's user/ident."""
        self.name = config.name
        """Connery's "real name", as used for whois."""

        self.channels = []
        """The list of channels Connery is currently in."""
    ...
    def log_raw(self, line, prefix):
        ...
        if not os.path.isdir(self.config.core.logdir):
            try:
                ...
                stderr('Please fix this and then run Connery again.')
                os._exit(1)
