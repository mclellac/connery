...
from connery.config import ConfigurationError
from connery.formatting import color, bold
from connery.logger import get_logger
from connery.module import commands, interval, priority, rule, OP
...
def setup(bot):
    ...
    bot.memory['safety_cache'] = connery.tools.ConneryMemory()
...
@connery.module.commands('safety')
...
@connery.module.interval(24 * 60 * 60)
def _clean_cache(bot):
    ...
    oldest_key_age = 0
    oldest_key = ''
    for key, data in connery.tools.iteritems(bot.memory['safety_cache']):
