...
in Connery modules. A number of things are moved into sub-modules like
``tools.time`` in 5.3; they will be removed from ``tools`` itself in 6.0.
"""

# tools.py - Connery misc tools
...
class ConneryMemory(dict):
    ...
    *Availability: 4.0; available as ``Connery.ConneryMemory`` in 3.1.0 - 3.2.0*
...
class ConneryMemoryWithDefault(defaultdict):
    """Same as ConneryMemory, but subclasses from collections.defaultdict."""
    def __init__(self, *args):
