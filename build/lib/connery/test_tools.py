...
test_tools.py - Connery misc tools
Copyright 2013, Ari Koivula, <ari@koivu.la>
Licensed under the Eiffel Forum License 2.

https://connery.dftba.net
"""
...
import connery.config
import connery.bot
import connery.irc
import connery.tools
import connery.trigger


class MockConnery(object):
    def __init__(self, nick, admin=False, owner=False):
        ...
        self.memory = connery.tools.ConneryMemory()

        self.ops = {}
        self.halfplus = {}
        self.voices = {}

        self.config = connery.config.Config('', load=False)
        self._init_config()
    ...
    def _init_config(self):
        ...
        home_dir = os.path.join(os.path.expanduser('~'), '.connery')
        if not os.path.exists(home_dir):
            os.mkdir(home_dir)
        cfg.parser.set('core', 'homedir', home_dir)


class MockConneryWrapper(object):
...
def get_example_test(tested_func, msg, results, privmsg, admin,
    ...
    Args:
        tested_func - A connery callable that accepts ConneryWrapper and Trigger.
    ...
    """
    def test():
        bot = MockConnery("NickName", admin=admin, owner=owner)

        match = None
        if hasattr(tested_func, "commands"):
            for command in tested_func.commands:
                regexp = connery.tools.get_command_regexp(".", command)
        ...
        pretrigger = connery.trigger.PreTrigger(bot.nick, full_message)
        trigger = connery.trigger.Trigger(bot.config, pretrigger, match)
        ...
        for _i in range(repeat):
            wrapper = MockConneryWrapper(bot, trigger)
